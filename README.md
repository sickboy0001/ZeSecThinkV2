- [ZeSecThinkV2](#zesecthinkv2)
  ...
  - [zstu\_tag\_descriptions](#zstu_tag_descriptions)
    ...
    - [SQL 実装コード](#sql-実装コード)
  - [1. なぜログを残すべきか](#1-なぜログを残すべきか)
  - [2. ログの登録先とテーブル定義案](#2-ログの登録先とテーブル定義案)
    - [`ai_Batches`](#ai_batches)
    - [ai\_Execution\_Logs](#ai_execution_logs)
    - [1. `api_version` の重要性](#1-api_version-の重要性)
    - [2. `duration_ms` (実行時間) の重要性](#2-duration_ms-実行時間-の重要性)
    - [ai\_Refinement\_History (投稿ごとの変更履歴)](#ai_refinement_history-投稿ごとの変更履歴)
  - [3. ログの登録タイミング（実装方法）](#3-ログの登録タイミング実装方法)
    - [登録のフロー](#登録のフロー)
    - [コードへの組み込みイメージ](#コードへの組み込みイメージ)
  - [4. UI（画面）への活用](#4-ui画面への活用)
  - [DDL](#ddl)
  - [プロンプトの保存](#プロンプトの保存)
    - [1. `prompt_templates` (親テーブル)](#1-prompt_templates-親テーブル)
    - [2. `prompt_versions` (子テーブル)](#2-prompt_versions-子テーブル)
  - [実装のポイント](#実装のポイント)
    - [① データの引き出し方（Next.js / FastAPI 共通）](#-データの引き出し方nextjs--fastapi-共通)
    - [② jsonb型の活用 (`model_config`)](#-jsonb型の活用-model_config)
    - [③ 編集機能（フロントエンド）の仕様](#-編集機能フロントエンドの仕様)
  - [todo](#todo)
  - [履歴](#履歴)
...
## スタイルに関する注意点
本プロジェクトでは、環境の互換性を優先するため、Tailwind CSS のカスタム値指定（Arbitrary values）を避け、標準クラスを使用しています。

- **NG**: className="w-[120px]" （ビルドエラーや適用されない可能性があります）
- **OK**: className="w-40" や className="w-32"
...
- [ ] stripe 対応
- [ ] twtter(x) 対応
- [ ] GoogleAnalysetic 対応　他アプリも、公開するものは対応したほうがいい- [x] AIログの登録
...
## 履歴
- 2026年3月7日
  - Posts Add　AIRefined Baddge 
- 2026年3月7日
  - update AI Refined to fixed site 
    - Day based , batch based
- 2026年3月2日
  - ProfileEdit
  - AILogList Add
- 2026年2月28日
  - Counter 見やすく
  - プロンプトについては別にマスタ化、テーブル化
- 2026年2月24日
  - Input回りデザイン調整
- 2026年2月21日
